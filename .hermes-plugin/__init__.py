...
BOOTSTRAP_MARKER = "dev-forge:using-dev-forge bootstrap for hermes"


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install <org>/dev-forge`): the plugin
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, "using-dev-forge", "SKILL.md")):
            return cand
    raise RuntimeError(
        "dev-forge plugin: cannot find the skills/ tree "
        f"(looked at {candidates}). Reinstall with "
        "`hermes plugins install <org>/dev-forge`."
    )
...
def _build_bootstrap(skills_dir: str) -> str:
    with open(
        os.path.join(skills_dir, "using-dev-forge", "SKILL.md"),
        encoding="utf-8",
    ) as f:
        body = _strip_frontmatter(f.read())

    tools_path = os.path.join(
        skills_dir, "using-dev-forge", "references", "hermes-tools.md"
    )
    ...
    return (
        ...
        f"{BOOTSTRAP_MARKER}\n\n"
        f"You have dev-forge.\n\n"
        f"The using-dev-forge skill content is included below and is already "
        f"loaded for this Hermes session. Follow it now. "
        f"Do not try to load using-dev-forge again.\n\n"
        f"{body}\n\n"
        f"## Loading dev-forge Skills on Hermes\n\n"
        f"dev-forge skills are registered with Hermes' native skill loader: "
        f'invoke one with `skill_view("dev-forge:skill-name")` '
        f'(for example `skill_view("dev-forge:forge-router")`). '
        f"If a namespaced lookup returns 'not found', read the skill file "
        f"directly instead:\n"
        f'`read_file("{skills_dir}/skill-name/SKILL.md")`\n\n'
        f"The dev-forge skills directory is: `{skills_dir}`\n\n"
        f"{tool_mapping}\n"
        f"</EXTREMELY_IMPORTANT>"
    )
