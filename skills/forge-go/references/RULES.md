...
- **R1.9 · 工件加载预算（任何阶段首轮强制）**
  ...
  dev-forge 的文件按加载策略分三类：
  | 类型 | 路径 | 长度 | 加载方式 |
  |---|---|---|---|
  | SPEC | `.specs/<id>/*.md` | 通常 < 200 行 | 整读 OK |
  | REFERENCE | `skills/<name>/references/*.md` 中的决策框架节选（tech-stacks-excerpt / ui-aesthetics / ui-anti-patterns / test-pyramid-excerpt 等）| 65~152 行 | **禁止默认整读**，按节 grep / read offset |
  | TEMPLATE | `skills/<name>/references/*.md` 中的产出模板（CHANGE / DESIGN / TASK / TEST / SUMMARY 等）| 28~222 行 | 按需整读（大模板只读当前块）|

  规则：
  1. **首轮预算**：进入任何阶段的首轮消息，加载的 reference/* 总行数 ≤ **150 行**（仅各 skill 自带的 `ui-anti-patterns.md`（65~84 行）可整读，其它必须按节读）
