...
- **维护者**：`forge-architect`（首次 / 重构）+ `forge-evolve`（增量同步 ADR）
- **首次创建**：<YYYY-MM-DD>
- **最近修订**：<YYYY-MM-DD>（来自 forge-evolve <YYYY-MM-DD> · 详见 `.specs/evolve/<date>-EVOLVE.md`）
...
> - `ARCHITECTURE.md`（**structure 层 · 本文件**）：模块图、跨模块契约、ADR 列表、扩展点、容量边界——**仅 2-design / forge-evolve 阶段加载**，约 200-600 行
...
> **新增 ADR 走 forge-evolve**——单 change 的「DESIGN § 9.2 项目级技术决策」段经用户 review 后，由 forge-evolve 转入此处。
> **改 ADR 状态走 forge-architect**——deprecate / supersede 涉及全局影响，需要重新评估。
...
| 日期 | 修改人 | 概要 | 工作流 |
|---|---|---|---|
| <YYYY-MM-DD> | forge-architect | 首次创建 | forge-architect 首跑 |
| <YYYY-MM-DD> | forge-evolve | ADR-002 新增（来自 add-cache-layer）| forge-evolve |
| <YYYY-MM-DD> | forge-architect | ADR-001 重审：保持 Postgres，否决 CockroachDB 迁移 | forge-architect 重构跑 |
