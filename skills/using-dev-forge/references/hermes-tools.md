...
To invoke a dev-forge skill, use:

```
skill_view("dev-forge:forge-router")
skill_view("dev-forge:forge-dev")
skill_view("dev-forge:forge-test")
```

If `skill_view` cannot find a dev-forge skill (it may not appear in the catalog
until the plugin fully registers it), fall back to reading the SKILL.md directly:

```
read_file(path="~/.hermes/plugins/dev-forge/skills/<skill-name>/SKILL.md")
```
