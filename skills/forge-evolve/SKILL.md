...
|---|---|---|---|
| **forge-evolve**（本文）| 把 change 级沉淀**单点 append** 到项目级文档 | 每月 / 每季 批量 | 仅 append ADR 列表 / 跨模块契约段 / 修订历史 |
...
- **forge-health 冗余巡检留的尾巴**：若 CONTEXT.md「清理窗口专列」或「技术债」段有标记，forge-evolve 扫描时**主动把对应的「既有抽象索引」条目拎出来让用户确认是否删**
...
- ADR / 跨模块契约 / Schema 总览 / 修订历史 → 只进 ARCHITECTURE（人读 / forge-design 读）
...
- 每段 append 在末尾加注释：`<!-- forge-evolve YYYY-MM-DD: from <change-id> -->`
...
- **遇到架构级冲突停下来**：例如某 § 9 要求 deprecate 现有 ADR → 提示用户跑 forge-architect
