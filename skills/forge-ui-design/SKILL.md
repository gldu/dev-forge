...
- **调性**：从 CHANGE.md「视觉调性」段读取已选项。**不允许在本阶段让用户重选。**
  - 「选定」非空（来源为预选 / 沿用既有 UI / 用户强偏好）→ 直接使用，**禁止**再出调性卡片
  - 无该字段，但仓库已有 `UI-DESIGN.md` 或实际 UI → 按 forge-change 0.55/0.6 沿用支自行补写 CHANGE 字段，**禁止**出卡片
  - 无该字段，且仓库无 UI / 无 UI-DESIGN（旧项目跳过了 0-change 的 greenfield）→ 才按调性卡片让用户选
  - 用户主动说"想换调性" → 整库换引导 `forge-restyle`；否则回 0-change 改 CHANGE.md
...
- CHANGE 已有「选定」或仓库已有 UI / UI-DESIGN 时，禁止再出调性卡片
